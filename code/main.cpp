...
private:
	...
	void createColorResources() {
		...
		createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat,
			VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);
	...
	void createTextureImageView() {
		...
		textureImageView = createImageView(textureImage, preferred8bitFormat, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
	}
	...
	void createTextureImage() {
		...
		createImage(static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight), mipLevels, VK_SAMPLE_COUNT_1_BIT,
			preferred8bitFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		...
		// TODO this is only if no mipmapping enabled
	...
	void createDescriptorSets() {
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			descriptorWrites[0].pTexelBufferView = nullptr; // TODO what is it?
	...
	void createFramebuffers() {
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++) {
			array<VkImageView, 3> attachments = { colorImageView, depthImageView, swapChainImageViews[i] };
	...
	void createDescriptorSetLayout() {
		...
		array<VkDescriptorSetLayoutBinding, 2> bindings = { uniformLayoutBinding, samplerLayoutBinding };
